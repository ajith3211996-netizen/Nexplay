...
class VLCPlayerView(context: ThemedReactContext) :
    ...
    fun setSource(src: ReadableMap) {
        val newUri = if (src.hasKey("uri")) src.getString("uri") else null
        val oldUri = if (srcMap?.hasKey("uri") == true) srcMap?.getString("uri") else null
        if (newUri != null && newUri == oldUri && mediaPlayer != null) {
            srcMap = src
            return
        }
        srcMap = src
        val autoplay = if (src.hasKey("autoplay")) src.getBoolean("autoplay") else true
        createPlayer(autoplay = autoplay)
    }
    ...
    fun releasePlayer() {
        ...
        abandonAudioFocus()
        stopNowPlaying()
        // Detached views still receive lifecycle callbacks, drop the resume
        // flag so onHostResume cannot restart a released player.
    ...
    override fun onSurfaceTextureAvailable(surface: SurfaceTexture, width: Int, height: Int) {
        ...
        val player = mediaPlayer
        if (player != null) {
            val saved = savedSurfaceTexture
            if (saved != null && !saved.isReleased) {
                if (saved != surface) {
                    try {
                        setSurfaceTexture(saved)
                    } catch (e: Exception) {
                        Log.e(TAG, "Error restoring saved SurfaceTexture, falling back to new surface", e)
            ...
            val vout = player.vlcVout
            if (!vout.areViewsAttached()) {
                vout.setVideoSurface(savedSurfaceTexture ?: surface)
                vout.attachViews(newVideoLayoutListener)
            }
        ...
        savedSurfaceTexture = surface
        // Rebuild on a re-attached surface, honoring the source's autoplay
        // and the paused prop.
        val autoplay = srcMap?.takeIf { it.hasKey("autoplay") }?.getBoolean("autoplay") ?: true
        createPlayer(autoplay = autoplay)
    }
    ...
    override fun onSurfaceTextureDestroyed(surface: SurfaceTexture): Boolean {
        if (mediaPlayer != null) {
            savedSurfaceTexture = surface
            // Returning false prevents Android from destroying the underlying
            // hardware SurfaceTexture. This preserves the native hardware
            // video decoder pipeline and prevents video blackscreen when user
            // minimizes/maximizes the player or rotates the screen.
            return false
        }
